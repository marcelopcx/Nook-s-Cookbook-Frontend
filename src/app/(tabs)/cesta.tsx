...
export default function CestaScreen() {
  ...
  const canAdvance = useMemo(() => {
    if (step === 0) {
      return recipeName.trim().length > 0 && description.trim().length > 0;
    }
    if (step === 1) {
          ...
          {saved ? (
            <View className="rounded-2xl border border-[#cfe4d9] bg-[#f0f7f3] p-4">
              ...
              <Text className="text-sm text-[#5c4a3d]">
                Tu receta se guardo correctamente. Puedes crear otra si lo
                deseas.
              </Text>
              <Pressable
                onPress={handleReset}
              ...
              >
                <Text className="font-bold text-white">Crear otra receta</Text>
              </Pressable>
            </View>
          ) : (
